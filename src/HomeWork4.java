 ...
 * 3. Что бы запустить тесты нажмите значок запуска кода напротив мтода main.
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for(String s: stringList) {
            ...
            for(int i = 0; i < s.length(); i++) {
                if(Character.isUpperCase(s.charAt(i))) valid = false;
            }
            if(valid) correctedList.add(s);
        }
        return correctedList;
    }
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        HashMap <String, Integer> sortedCart = new HashMap<>();
        for(String s: shopCart.keySet()) {
            if(shopCart.get(s) <= 300) sortedCart.put(s, shopCart.get(s));
        }
        return sortedCart;
    }
